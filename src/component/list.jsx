...
const List = ({ todo, deleteTodo, doneTodo, editTodo, editTodoValue, getValue }) => {
    if(todo == ''){
        todo = [];
    }
        ...
        <div className='container-listTodo' >
            {todo.map((todoItem, index) => (
                <div className='listTodo' key={index}>
                    <input type="text" className={`todo-item ${todoItem[0].status ? "coret" : ''} ${todoItem[0].setEdit ? "editing" : ''}`} disabled={todoItem[0].setEdit ? null : 'disabled'} value={todoItem[0].setEdit ? getValue : todoItem[0].todo} onChange={(e) => edit(e)} />
